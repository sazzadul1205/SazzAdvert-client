...
import UpdateBigTestimonials from "./UpdateBigTestimonials/UpdateBigTestimonials";
import AddBigTestimonials from "./AddBigTestimonials/AddBigTestimonials";

const AdminBigTestimonials = () => {
  ...
  const {
    data: BigTestimonialsData,
    isLoading: SlidersLoading,
    error: SlidersError,
    refetch: TestimonialsRefetch,
  } = useQuery({
    queryKey: ["BigTestimonialsData"],
    queryFn: async () => {
  ...
  const handleDelete = async (itemId, itemName) => {
    try {
      ...
      if (result.isConfirmed) {
        await axiosPublic.delete(`/BigTestimonials/${itemId}`);
        TestimonialsRefetch();
        showSuccessAlert(
          "Item Deleted!",
      ...
      <div className="mt-2">
        ...
        <div className="overflow-x-auto  bg-yellow-50 rounded-xl p-2">
          ...
          <table className="table w-full rounded-xl ">
            ...
            <tbody>
              {BigTestimonialsData.map((card) => (
                <tr key={card.id} className="py-8 px-4">
                  <td className="py-2">
                    <img src={card.image} alt={card.image} className="" />
                  </td>
                  <td className="py-2 px-4">{card.content}</td>
                      ...
                      <div>
                        ...
                        >
                          <div className="modal-box bg-white">
                            ...
                            <UpdateBigTestimonials
                              key={card._id}
                              BigTestimonials={card} // Pass the entire whatWeDo object
...
export default AdminBigTestimonials;
