...
const AdminFAQ = () => {
  ...
  // Loading state
  if (titleDataLoading || FAQLoading) {
    return <Loader />;
  }

  // Error state
  if (titleDataError || FAQError) {
  ...
  const handleDelete = async (itemId, itemName) => {
    try {
      ...
      if (result.isConfirmed) {
        await axiosPublic.delete(`/FAQ/${itemId}`);
        FAQRefetch();
        showSuccessAlert(
          "Item Deleted!",
            ...
            <tbody>
              {FAQ.map((card) => (
                <tr key={card.id} className="py-5 px-4">
                  <td className="font-bold ">{card.question}</td>
                  <td className="py-2 px-4">{card.answer}</td>
