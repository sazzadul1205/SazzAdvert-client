...
import AddGetInTouch from "./AddGetInTouch/AddGetInTouch";
import UpdateGetInTouch from "./UpdateGetInTouch/UpdateGetInTouch";
...
const AdminGetInTouch = () => {
  ...
  // Fetching GetInTouch Cards Data
  const {
    data: GetInTouch,
    isLoading: GetInTouchLoading,
    error: GetInTouchError,
    refetch: GetInTouchRefetch,
  } = useQuery({
    queryKey: ["GetInTouch"],
    queryFn: async () => {
  ...
  // Loading state
  if (titleDataLoading || GetInTouchLoading) {
    return <Loader />;
  }

  // Error state
  if (titleDataError || GetInTouchError) {
  ...
  const handleDelete = async (itemId, itemName) => {
    try {
      ...
      if (result.isConfirmed) {
        await axiosPublic.delete(`/GetInTouch/${itemId}`);
        GetInTouchRefetch();
        showSuccessAlert(
          "Item Deleted!",
            ...
            <tbody>
              {GetInTouch.map((card) => (
                <tr key={card.id} className="py-5 px-4">
                  <td className="font-bold ">{card.title}</td>
                  <td className="py-2 px-4">{card.location}</td>
                  <td className="py-2 w-28">{card.type}</td>
                  <td className="py-5 w-40  font-normal">
                    {card.date}
                  </td>
                      ...
                      <div>
                        ...
                        >
                          <div className="modal-box bg-white">
                            ...
                            <UpdateGetInTouch
                              key={card._id}
                              GetInTouch={card} // Pass the entire whatWeDo object
