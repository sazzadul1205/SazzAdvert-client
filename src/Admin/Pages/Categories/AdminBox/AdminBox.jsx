...
const AdminBox = () => {
  ...
  const {
    data: BannerCategories2 = {},
    isLoading,
    refetch,
  } = useQuery({
    queryKey: ["BannerCategories2"],
    queryFn: async () => {
      const res = await axiosPublic.get(`/Banners?page=Categories2`);
      return res.data[0];
    },
  });
    ...
    <div>
      ...
      <div className="flex justify-between pb-5 border-b border-red-500">
        ...
        <button
          ...
          onClick={() => handleEditClick(BannerCategories2)}
        >
      ...
      <div className="bg-blue-100 rounded-xl mt-5">
        ...
        <div className="p-5">
          <div className="text-center">
            {BannerCategories2.postedBy && (
              <div className="flex w-[200px] mx-auto">
                <img
                  src={BannerCategories2.postedBy.logo}
                  className="ml-5 w-8 h-8"
                  alt="Posted by logo"
                />
                <div className="ml-5">
                  <p>Posted by</p>
                  <p className="font-bold">{BannerCategories2.postedBy.name}</p>
                </div>
              </div>
            )}
            <div className="pt-4 mt-5">
              {BannerCategories2.buttons &&
                BannerCategories2.buttons.map((buttonText, index) => (
                  <button
                    key={index}
            ...
            <div className="mt-5">
              <h2 className="text-3xl font-bold mb-5">
                {BannerCategories2.title}
              </h2>
            </div>
          </div>
          <div className="mx-auto w-[500px]">
            {BannerCategories2.bannerImage && (
              <img
                src={BannerCategories2.bannerImage}
