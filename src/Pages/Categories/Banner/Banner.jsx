...
const Banner = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosPublic.get(`/Banners?page=Categories`);
      return res.data[0];
    },
  });
    ...
    <div className="bg-[#FFE6E6]">
      <div className="flex justify-between max-w-[1200px] mx-auto h-[720px] pt-10 items-center gap-5">
        ...
        <div className="text-black w-[535px]">
          <p className="text-lg mb-4 font-semibold">{CategoriesBanner.title}</p>
          <h1 className="text-[34px] font-bold mb-6">
            {CategoriesBanner.description}
          </h1>
          <h1 className="text-[16px] mb-12">{CategoriesBanner.content}</h1>
          <NavLink to={"/AboutUs"}>
        ...
        <div className="relative w-[522px] h-[365px]">
          ...
          <img
            src={CategoriesBanner.imageUrl}
            alt={CategoriesBanner.description}
