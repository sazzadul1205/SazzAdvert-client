...
const Banner = () => {
  ...
  const {
    data: TermsConditionsBanner,
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["TermsConditionsBanner"],
    queryFn: async () => {
      const res = await axiosPublic.get(`/Banners?page=TermsConditions`);
      return res.data[0];
    },
  });
  ...
  return (
    <div className="bg-[#FFE6E6]">
      <div className="flex justify-between max-w-[1200px] mx-auto  pt-52 items-center gap-5">
        {/* Text Section */}
        <div className="text-black w-[535px]">
          <p className="text-lg mb-4 font-semibold">
            {TermsConditionsBanner.title}
          </p>
          <h1 className="text-[34px] font-bold mb-6">
            {TermsConditionsBanner.description}
          </h1>
          <h1 className="text-[16px] mb-12">{TermsConditionsBanner.content}</h1>
          <NavLink to={"/AboutUs"}>
        ...
        <div className="relative w-[522px] h-[365px]">
          ...
          <img
            src={TermsConditionsBanner.imageUrl}
            alt={TermsConditionsBanner.description}
